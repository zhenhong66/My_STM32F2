...
USBH_HCD usbh0_hcd = {                  /* Host Controller Driver structure   */
  512,                                  /* Maximum data that can be handled   */
  10000,                                /* Number of NAKs for ctrl endpoints  */
  1000000,                              /* Number of NAKs for bulk endpoints  */
...
static BOOL      HW_Disconnected[2]                = { __FALSE };
...
static BOOL USBH_STM32_CH_Disable (USBH_STM32_CH *ptrCH) {
  ...
  if (ptrCH->HCCHAR & USBH_STM32_HCCHAR_CHENA) {
    ...
    ptrCH->HCCHAR    =  (ptrCH->HCCHAR  |  USBH_STM32_HCCHAR_CHENA) | USBH_STM32_HCCHAR_CHDIS;
    for (tout = 10; tout >= 0; tout--) {          /* Wait max 100 ms          */
...
static BOOL USBH_STM32_CH_TransferEnqueue (U8 ctrl, USBH_STM32_CH *ptrCH, U32 tgl_typ, U8 *buf, U32 len) {
  ...
  if (((tgl_typ & 0x0F) == USBH_PACKET_OUT  ) ||  /* If packet is OUT or SETUP*/
      ((tgl_typ & 0x0F) == USBH_PACKET_SETUP)  ) {
                                                  /* Load data to be sent     */
    DFIFO  = OTG_DFIFO[ctrl][ch_idx];
    ptrU32 = (U32 *)(buf);
    i      = (len + 3) / 4;
...
#elif defined(STM3220F)
      usbh_stm32_cfg_pin (PORT_H,  5, (1)|(0<<11));         /* PH5:  OTG_FS_PS*/
#else
...
BOOL USBH_STM32_Init (U8 ctrl, BOOL on) {
  ...
  if (on) {
    ...
    ptr_OTG->GUSBCFG   &=~(USBH_STM32_GUSBCFG_FHMOD   |   /* Clear register   */
    ...
    if (!ctrl) {
      ptr_OTG->GUSBCFG |=  USBH_STM32_GUSBCFG_PHYSEL;     /* Full-spd trnscvr */
    }
#ifndef STM3220F
    else {
...
BOOL USBH_STM32_Port_Reset (U8 ctrl, U8 port) {
  ...
  for (tout = 10; tout >= 0; tout--) {                /* Wait for max 100 ms  */
...
U32 USBH_STM32_Get_Connect (U8 ctrl) {
  ...
  ret  = Port_Discon_Evt[ctrl] << 16;

  if (ret || ((stat ^ Port_Con[ctrl]) && (Port_Con[ctrl]))){/* If port discon */
    Port_Discon_Evt[ctrl] = __FALSE;
    return (0x10000);
  }
  ...
  if (cntDebounce[ctrl]) {
    ...
    if (!cntDebounce[ctrl]) {           /* If debounce time expired           */
      if ((stat ^ Port_Con[ctrl]) && (!Port_Con[ctrl])) { 
  ...
  if (!cntDebounce[ctrl] && (stat ^ Port_Con[ctrl]) && (!Port_Con[ctrl])) { 
    cntDebounce[ctrl] = DEBOUNCE_250ms;
  }
...
void OTG_xS_IRQHandler (U8 ctrl) {
  ...
  if (disconn) {                                  /* If disconnect detected   */
    HW_Disconnected[ctrl] = __TRUE;
    Port_Discon_Evt[ctrl] = __TRUE;
    Port_Con[ctrl]        = __FALSE;
    ptr_CH = (USBH_STM32_CH *)(&(ptr_OTG->HCCHAR0));
    for (ch = 0; ch < max_ch; ch++) {
      if (CHURB[ctrl][ch]) {
        ptr_URB = CHURB[ctrl][ch];
        ptr_CH->HCINTMSK       = 0;
        ptr_URB->Submitted     = 0;
        ptr_URB->InProgress    = 0;
        ptr_URB->Completed     = 1;
        ptr_URB->TimeoutCount  = 0;
        ptr_URB->Timeout       = 1;
        CHURB[ctrl][ch]        = 0;
      }
      ptr_CH++;
      ...
      if (haint & (1 << ch)) {                    /* If channels interrupt act*/
        ...
        if ((ptr_URB->PacketType == USBH_PACKET_OUT) ||     /* If OUT packet  ----------*/
          ...
          if (hcint & USBH_STM32_HCINT_XFRC) {
            ...
            ptr_URB->ResponsePacketType = USBH_PACKET_NAK;
            if ((!HW_Disconnected[ctrl]) && (ptr_CH->EPTYP != USB_ENDPOINT_TYPE_INTERRUPT) && (CHURB[ctrl][ch]->NAKRetries--)) {
                                                  /* Reenable channel         */

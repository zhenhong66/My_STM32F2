...
/* Configure the LCD Control pins --------------------------------------------*/
  ...
  //PE.3(A19 (RS))
  GPIOE->MODER    &= ~0x000000C0;       /* Clear Bits                         */
  GPIOE->MODER    |=  0x00000080;       /* Alternate Function mode            */
  GPIOE->OSPEEDR  &= ~0x000000C0;       /* Clear Bits                         */
  GPIOE->OSPEEDR  |=  0x00000080;       /* 50 MHz Fast speed                  */
  GPIOE->AFR[0]   &= ~0x0000F000;       /* Clear Bits                         */
  GPIOE->AFR[0]   |=  0x0000C000;       /* Alternate Function mode AF12       */

#ifdef __STM_EVAL                       /* STM3220G-EVAL and STM3240G-EVAL    */
  /* PD.7(NE1 (LCD/CS)) - NOR/PSRAM */
  GPIOD->MODER    &= ~0x0000C000;       /* Clear Bits                         */
  GPIOD->MODER    |=  0x00008000;       /* Alternate Function mode            */
  GPIOD->OSPEEDR  &= ~0x0000C000;       /* Clear Bits                         */
  GPIOD->OSPEEDR  |=  0x00008000;       /* 50 MHz Fast speed                  */
  GPIOD->AFR[0]   &= ~0xF0000000;       /* Clear Bits                         */
  GPIOD->AFR[0]   |=  0xC0000000;       /* Alternate Function mode AF12       */

  /* PE.02(LCD Backlight                                                      */
  GPIOE->BSRRH    |=  (1UL <<   2);     /* Backlight off                      */
  GPIOE->MODER    &= ~(3UL << 2*2);     /* Clear Bits                         */
  GPIOE->MODER    |=  (1UL << 2*2);     /* PC.2 is output                     */
  GPIOE->OTYPER   &= ~(1UL <<   2);     /* PC.2 is output Push-Pull           */
  GPIOE->OSPEEDR  &= ~(3UL << 2*2);     /* Clear Bits                         */
  GPIOE->OSPEEDR  |=  (2UL << 2*2);     /* PC.2 is 50MHz Fast Speed           */
...
#ifdef __STM_EVAL                       /* STM3220G-EVAL and STM3240G-EVAL    */
  FSMC_Bank1->BTCR[(1-1)*2 + 1] =       /* Bank3 NOR/SRAM timing register     */
                                        /* configuration                      */
#else                                   /* MCBSTM32F200 and MCBSTMF400 board  */
...
#ifdef __STM_EVAL                       /* STM3220G-EVAL and STM3240G-EVAL    */
  GPIOE->BSRRL |= (1UL << 2);           /* Backlight on                       */
#else                                   /* MCBSTM32F200 and MCBSTMF400 board  */

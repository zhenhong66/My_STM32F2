...
0x69,0x74,0x6c,0x65,0x3e,0x53,0x54,0x4d,0x33,0x32,0x46,0x32,0x78,0x37,0x54,0x41,
...
0x6c,0x69,0x63,0x3b,0x22,0x3e,0x53,0x54,0x4d,0x33,0x32,0x46,0x32,0x78,0x37,0x20,
...
0x3d,0x22,0x2f,0x53,0x54,0x4d,0x33,0x32,0x46,0x32,0x78,0x37,0x2e,0x68,0x74,0x6d,
...
0x20,0x68,0x72,0x65,0x66,0x3d,0x22,0x53,0x54,0x4d,0x33,0x32,0x46,0x32,0x78,0x37,
...
0x0a,0x20,0x68,0x72,0x65,0x66,0x3d,0x22,0x2f,0x53,0x54,0x4d,0x33,0x32,0x46,0x32,
...
void http_server_serve(int conn) 
{
  ...
  if (strncmp((char *)recv_buffer,"GET /STM32F2x7_files/ST.gif",27)==0)
  {
    file = fs_open("/STM32F2x7_files/ST.gif"); 
    write(conn, (const unsigned char*)(file->data), (size_t)file->len);
    if(file) fs_close(file);
  }
  /* Check if request to get stm32.jpeg */
  else if (strncmp((char *)recv_buffer,"GET /STM32F2x7_files/stm32.jpeg",31)==0)
  {
    file = fs_open("/STM32F2x7_files/stm32.jpeg"); 
    write(conn, (const unsigned char*)(file->data), (size_t)file->len);
    if(file) fs_close(file);
  }
  /* Check if request to get ST logo.jpeg */
  else if (strncmp((char *)recv_buffer,"GET /STM32F2x7_files/logo.jpg", 29) == 0)
  {
    file = fs_open("/STM32F2x7_files/logo.jpg"); 
    write(conn, (const unsigned char*)(file->data), (size_t)file->len);
    if(file) fs_close(file);
  }
  else if(strncmp((char *)recv_buffer, "GET /STM32F2x7TASKS.html", 24) == 0)
  {
    /* Load dynamic page */
    DynWebPage(conn);
  }
